...
@given('Seleccionar solicitud que cuenta con el estatus "{estatus}" revertir depósito')
...
def step_impl(context):
    ...
    context.revertir_page.seleccionar_menu_revertir_dep()

@when('Revertir depósito de la comisión y aceptar')
def step_impl(context):

    context.revertir_page.confirmar_revertir_depósito()

@then('Validar el estatus de la comisión "{estatus}" revertir depósito de la comisión')
def step_impl(context,estatus):
    time.sleep(1)
    context.all_page.menu_comision()
    time.sleep(1)
