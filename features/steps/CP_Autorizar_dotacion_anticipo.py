...
@given('Seleccionar solicitud que cuenta con el estatus "{estatus}" autorizar dotación con anticipo')
...
@when('Autorizar la solicitud y aceptar autorizar dotación con anticipo')
def step_impl(context):

    context.autorizar_page.confirmar_autorizar_con()

@then('Validar el estatus de la comisión "{estatus}" autorizar dotación con anticipo')
def step_impl(context,estatus):
    time.sleep(1)
    context.all_page.menu_comision()
    time.sleep(1)
